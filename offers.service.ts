...
export async function publishOffer(input: PublishInput) {
  const p = await pool.query(
    `SELECT participant_type, rating, status FROM projection.participants WHERE participant_id = $1`,
  ...
  const r = await pool.query(
    `INSERT INTO projection.offers_state (
  ...
  return { offer_id: r.rows[0].offer_id as string };
}

export async function withdrawOffer(offerId: string) {
  const r = await pool.query(
    `UPDATE projection.offers_state SET status = 'withdrawn'
...
export async function listMarketOffers() {
  const r = await pool.query(
    `SELECT o.offer_id, o.metal, o.side, o.price_rub_per_gram, o.volume_grams,
...
export async function listMyOffers(participantId: string) {
  const r = await pool.query(
    `SELECT o.*, p.company_name, p.email, p.phone
     FROM projection.offers_state o
     LEFT JOIN projection.participants p ON o.participant_id = p.participant_id
