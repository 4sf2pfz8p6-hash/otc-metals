...
exports.currentCandidate = currentCandidate;
exports.respondToCandidate = respondToCandidate;
exports.revealContacts = revealContacts;
exports.getContacts = getContacts;
exports.setResult = setResult;
exports.listMyDeals = listMyDeals;
...
async function getParticipant(id) {
    const r = await db_1.pool.query(`SELECT participant_id, company_name, contact_person, phone, email,
            participant_type, status, deals_completed, created_at
     FROM projection.participants WHERE participant_id = $1`, [id]);
    return r.rows[0] || null;
}
async function getOffer(offerId) {
    const r = await db_1.pool.query(`SELECT * FROM projection.offers_state WHERE offer_id = $1`, [offerId]);
    return r.rows[0] || null;
}
async function notify(participantId, type, dealId, offerId) {
    await db_1.pool.query(`INSERT INTO projection.notifications (participant_id, deal_id, offer_id, type)
     VALUES ($1, $2, $3, $4)`, [participantId, dealId, offerId, type]);
}
function counterpartyFacts(p) {
    return {
...
async function currentDealRow(offerId) {
    const active = await db_1.pool.query(`SELECT * FROM projection.deals
     WHERE offer_id = $1 AND status = ANY($2)
     ORDER BY created_at ASC LIMIT 1`, [offerId, ["confirmed", "reveal_pending", "contacts_revealed", "awaiting_confirmation"]]);
    if (active.rows[0])
        return active.rows[0];
    const next = await db_1.pool.query(`SELECT * FROM projection.deals
     WHERE offer_id = $1 AND status = 'interest_sent'
     ORDER BY created_at ASC LIMIT 1`, [offerId]);
    return next.rows[0] || null;
}
async function promoteNext(offerId) {
    const next = await db_1.pool.query(`SELECT deal_id, owner_id FROM projection.deals
     WHERE offer_id = $1 AND status = 'interest_sent'
     ORDER BY created_at ASC LIMIT 1`, [offerId]);
    if (next.rows[0]) {
...
async function createInterest(offerId, responderId) {
    ...
    const cnt = await db_1.pool.query(`SELECT COUNT(*)::int AS c FROM projection.deals
     WHERE offer_id = $1 AND status = ANY($2)`, [offerId, ACTIVE_STATUSES]);
    if (cnt.rows[0].c >= DEAL_LIMIT) {
        throw new Error("Приём откликов по этому ордеру закрыт");
    }
    const pos = await db_1.pool.query(`SELECT COUNT(*)::int AS c FROM projection.deals WHERE offer_id = $1`, [offerId]);
    try {
        const r = await db_1.pool.query(`INSERT INTO projection.deals
         (offer_id, owner_id, responder_id, status, queue_position)
       VALUES ($1, $2, $3, 'interest_sent', $4)
       RETURNING deal_id`, [offerId, offer.participant_id, responderId, pos.rows[0].c + 1]);
...
async function currentCandidate(offerId, ownerId) {
    const offer = await getOffer(offerId);
    if (!offer)
        throw new Error("Оффер не найден");
    if (offer.participant_id !== ownerId)
...
async function respondToCandidate(dealId, ownerId, action) {
    const r = await db_1.pool.query(`SELECT * FROM projection.deals WHERE deal_id = $1`, [dealId]);
    const d = r.rows[0];
    if (!d)
    ...
    if (d.status !== "interest_sent")
        throw new Error("Кандидат уже обработан");
    if (action === "confirm") {
        await db_1.pool.query(`UPDATE projection.deals SET status = 'confirmed', is_active = true, responded_at = now()
       WHERE deal_id = $1`, [dealId]);
        await notify(d.responder_id, "confirmed", dealId, d.offer_id);
        return { status: "confirmed" };
    }
    else if (action === "decline") {
        await db_1.pool.query(`UPDATE projection.deals SET status = 'declined', is_active = false WHERE deal_id = $1`, [dealId]);
...
async function revealContacts(dealId, userId) {
    const r = await db_1.pool.query(`SELECT * FROM projection.deals WHERE deal_id = $1`, [dealId]);
    const d = r.rows[0];
    if (!d)
        throw new Error("Сделка не найдена");
    if (d.owner_id !== userId && d.responder_id !== userId)
        throw new Error("Нет доступа");
    ...
    await db_1.pool.query(`UPDATE projection.deals SET ${field} = true WHERE deal_id = $1`, [dealId]);
    const upd = await db_1.pool.query(`SELECT * FROM projection.deals WHERE deal_id = $1`, [dealId]);
    const nd = upd.rows[0];
    if (nd.owner_reveal && nd.responder_reveal) {
        await db_1.pool.query(`UPDATE projection.deals SET status = 'contacts_revealed', revealed_at = now() WHERE deal_id = $1`, [dealId]);
        ...
        await db_1.pool.query(`UPDATE projection.deals SET status = 'reveal_pending' WHERE deal_id = $1`, [dealId]);
...
async function getContacts(dealId, userId) {
    const r = await db_1.pool.query(`SELECT * FROM projection.deals WHERE deal_id = $1`, [dealId]);
    const d = r.rows[0];
    if (!d)
        throw new Error("Сделка не найдена");
...
async function setResult(dealId, userId, result) {
    const r = await db_1.pool.query(`SELECT * FROM projection.deals WHERE deal_id = $1`, [dealId]);
    const d = r.rows[0];
    if (!d)
        throw new Error("Сделка не найдена");
    ...
    if (result === "cancelled") {
        await db_1.pool.query(`UPDATE projection.deals SET ${field} = 'cancelled', status = 'cancelled', is_active = false, closed_at = now()
       WHERE deal_id = $1`, [dealId]);
        const other = isOwner ? d.responder_id : d.owner_id;
    ...
    if (result === "completed") {
        await db_1.pool.query(`UPDATE projection.deals SET ${field} = 'completed' WHERE deal_id = $1`, [dealId]);
        const upd = await db_1.pool.query(`SELECT * FROM projection.deals WHERE deal_id = $1`, [dealId]);
        const nd = upd.rows[0];
        if (nd.owner_result === "completed" && nd.responder_result === "completed") {
            await db_1.pool.query(`UPDATE projection.deals SET status = 'completed', is_active = false, closed_at = now() WHERE deal_id = $1`, [dealId]);
            await db_1.pool.query(`UPDATE projection.participants SET deals_completed = COALESCE(deals_completed,0) + 1
         WHERE participant_id = ANY($1)`, [[d.owner_id, d.responder_id]]);
            await db_1.pool.query(`UPDATE projection.deals SET status = 'expired_queue'
         WHERE offer_id = $1 AND status = 'interest_sent'`, [d.offer_id]);
            await db_1.pool.query(`UPDATE projection.offers_state SET status = 'executed' WHERE offer_id = $1`, [d.offer_id]);
            await notify(d.owner_id, "completed", dealId, d.offer_id);
            await notify(d.responder_id, "completed", dealId, d.offer_id);
            return { status: "completed" };
        }
        else {
            await db_1.pool.query(`UPDATE projection.deals SET status = 'awaiting_confirmation' WHERE deal_id = $1`, [dealId]);
            const other = isOwner ? d.responder_id : d.owner_id;
...
async function listMyDeals(userId) {
    const r = await db_1.pool.query(`SELECT d.*, o.metal, o.purity, o.side, o.price_rub_per_gram, o.volume_grams,
...
async function listNotifications(userId) {
    const r = await db_1.pool.query(`SELECT notif_id, type, deal_id, offer_id, is_read, created_at
...
async function markNotificationRead(notifId, userId) {
    await db_1.pool.query(`UPDATE projection.notifications SET is_read = true
     WHERE notif_id = $1 AND participant_id = $2`, [notifId, userId]);
    return { ok: true };
}
