...
function explainDbError(err) {
    ...
    if (code === "28P01" || /password authentication failed/i.test(message)) {
        return ("[db] Неверный пароль в DATABASE_URL. Supabase → Database → Reset database password, " +
...
if (!connectionString) {
    console.error("[db] DATABASE_URL не задан — API не сможет читать офферы. Задайте переменную в Render.");
}
else {
    ...
    exports.pool
        ...
        if (r.rows[0]?.exists) {
            ...
            console.error("[db] Таблица projection.offers_state отсутствует — выполните npm run migrate");
        }
    })
